...
class MVSDatasetDTU(Dataset):
    def __init__(self, root_dir, config_dir, split, n_views=3, levels=1,
                 img_wh=None, downSample=1.0, max_len=-1, closest_views=False):
        """
    ...
    def build_metas(self):
        self.metas = []
        with open(f'{self.config_dir}/lists/dtu_{self.split}_all.txt') as f:
        ...
        for scan in self.scans:
            with open(f'{self.config_dir}/dtu_pairs.txt') as f:
    ...
    def __getitem__(self, idx):
        ...
        nearest_pose_ids = get_nearest_pose_ids(self.cam2worlds[target_view],
                                                self.cam2worlds,
                                                len(self.cam2worlds),
                                                tar_id=target_view,
        ...
        for i, vid in enumerate(view_ids):
            ...
            img_wh = self.img_wh if self.img_wh else np.round(np.array(img.size) * self.downSample).astype('int')
